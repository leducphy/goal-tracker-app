...
class AuthService {
  async login(credentials: LoginRequest): Promise<ApiResponse<LoginResponse>> {
    try {
      ...
      console.log(
        "🔐 Login response data access_token_expire:",
        data.access_token_expire
      );
      console.log(
        "🔐 Login response data refresh_token_expire:",
    ...
    try {
      ...
      if (token) {
        try {
          ...
          console.warn(
            "⚠️ Logout endpoint failed, continuing with local cleanup"
          );
        }
      }
  ...
  async updateProfile(
    ...
    try {
      ...
      const response = await httpClient.put<UserData>(
  ...
  async googleLogin(idToken: string): Promise<ApiResponse<LoginResponse>> {
    try {
      ...
      const googleLoginUrl = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.GOOGLE_LOGIN}`;
